...
class PromptGenerator(nn.Module):
    def __init__(
        ...
        # self.conv4 = nn.Conv2d(12,3,1)
        # self.conv5 = nn.Conv2d(9, 3, 1)
    ...
    def forward(self, x):
        if self.input_type == "laplacian":
            ...
            for x_i in x[1:]:
                x_i = F.interpolate(
                    x_i,
                    size=(laplacian.size(2), laplacian.size(3)),
                    mode="bilinear",
                    align_corners=True,
                )
    ...
    def fft_old(self, x, rate):
        ...
        print(f"Shape of fft: {fft.shape}")
        ...
        print(f"shape of fft after shift: {fft_hires.shape}")
    ...
    def fft(self, x, rate):
        ...
        Yl, Yh = xfm(resize(x))
        #
        # Yh is the high frequency component of the input image
        # In order to return it I can simply use the Yh coefficients and add them all together and return it.

        # ----------------------------------------------------
        # NOTE: for wavelets other than haar
        #high_freq_feat = (
        #    Yh[0][:, :, 0, 1:-1, 1:-1]
        #    + Yh[0][:, :, 1, 1:-1, 1:-1]
        #    + Yh[0][:, :, 2, 1:-1, 1:-1]
        ...
        # print(f"Shape of Yh :{Yh[0].shape }")
        # print(f"Shape of Yh :{Yh[0][:, :, 0, 1:-1, 1:-1].shape }")
        #
        # ll = torch.abs(Yl)
        # lh = torch.abs(Yh[0][:, :, 0, :, :])
        # hl = torch.abs(Yh[0][:, :, 1, :, :])
        ...
        # ll = self.conv0(Yl)
        # lh = self.conv1(Yh[0][:, :, 0, :, :])
        # hl = self.conv2(Yh[0][:, :, 1, :, :])
        # hh = self.conv3(Yh[0][:, :, 2, :, :])
        # high_freq_feat = self.conv4(torch.cat((ll, lh, hl, hh), dim=1))

        # m1 = self.conv0(torch.abs(2 * x - ll))
        # m2 = self.conv1(torch.max(lh, hl))
        # m3 = self.conv2(hh)
        #
        # high_freq_feat = self.conv5(torch.cat((m1, m2, m3), dim=1))
        #high_freq_feat = high_freq_feat[:,:, 3:-3, 3:-3]
        #import pdb;pdb.set_trace()
        assert high_freq_feat.shape == x.shape
        return high_freq_feat
