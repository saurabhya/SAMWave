...
def eval_psnr(loader, model, eval_type=None):
    ...
    if eval_type == 'f1':
        ...
        metric1, metric2, metric3, metric4 = 'iou', 'dice', 'none', 'none'
    ...
    return result1, result2, result3, result4, metric1, metric2, metric3, metric4, torch.stack(losses_infer).mean()
...
def main(config_, save_path, args):
    ...
    config = config_
    wandb.init(project="Sam Adapter training")
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            with torch.no_grad():
                result1, result2, result3, result4, metric1, metric2, metric3, metric4, infer_loss = eval_psnr(val_loader, model,
                ...
                if local_rank == 0:
                    ...
                    log_info.append('val: infer_loss ={:.4f}'.format(infer_loss))
                    writer.add_scalars(metric4, {'val': infer_loss}, epoch)
        ...
        wandb.log(
            {
                ...
                "train G loss": train_loss_G,
                "epoch": epoch,
                "eval_loss": infer_loss
            }
        )
    wandb.finish()
