...
def calc_polyp(y_pred, y_true, epsilon=1e-6):
    ...
    with torch.no_grad():
        assert y_pred.shape == y_true.shape
        for i in range(batchsize):
            true, pred = y_true[i, 0].cpu(), y_pred[i, 0].cpu()
            pred_flat = pred.float()
            true_flat = true.float()
            intersection = torch.sum(pred_flat * true_flat)
            total = torch.sum(pred_flat) + torch.sum(true_flat)
            union = total.data - intersection.data
            #import pdb;pdb.set_trace()
            dic = (2. * intersection.data + epsilon) / (total.data + epsilon)
            dice.append(dic.tolist())
            io = (intersection.data + epsilon) / (union + epsilon)
            iou.append(io.tolist())
    return np.mean(dice), np.mean(iou), np.array(0), np.array(0)
'''
def calc_polyp(y_pred, y_true):
    smooth = 1
    batchsize = y_true.shape[0]
    with torch.no_grad():
        assert y_pred.shape == y_true.shape
        ...
        for i in range(batchsize):
            DSC, IOU = 0, 0
            input = y_pred[i, 0].cpu().data.numpy()
            target = y_true[i, 0].cpu().data.numpy()
            input = np.where(input > min(1.5 * np.mean(input), 255), np.ones_like(input),
                             np.zeros_like(input))
            target = np.where(target > min(1.5 * np.mean(target), 255), np.ones_like(target),
                              np.zeros_like(target))

            # dice
            input_flat = np.reshape(input, (-1))
            target_flat = np.reshape(y_true, (-1))
            intersection = (input_flat * target_flat)
            dice = (2 * intersection.sum() + smooth) / (input.sum() + y_true.sum() + smooth)
            dice = '{:.4f}'.format(dice)
            dice = float(dice)
...
def calc_kvasir(pred, target):
    ...
    return v_dice, v_iou, np.array(0), np.array(0)